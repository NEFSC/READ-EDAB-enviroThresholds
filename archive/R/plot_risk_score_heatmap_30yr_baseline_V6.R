# plot_risk_score_heatmap_30yr_baseline_V6.R
#
# Purpose: Generate a heatmap table visualization of the V6 thermal habitat 
#          risk scores over time using the 30-Year Fixed Baseline method. 
#
...
#
# Output:
#   Plots : images/scoring/risk_score_heatmap_V6_30yr_baseline.png
#
...
score_file <- here::here("data/scoring/risk_scores_hindcast_V6_30yr_baseline.rds")

if (!file.exists(score_file)) {
  stop("Score data not found. Run thermal_suitability_z_scoring_30yr_baseline_V6.R first.")
}
...
message("Generating 30-year fixed baseline risk score heatmap...")
...
p_heatmap <- risk_scores |>
  ...
  labs(
    title = "Thermal Habitat Risk Scores: 30-Year Historical Baseline (V6)",
    subtitle = "Green (Negative) = Less Risk Averse | White (0) = Neutral | Red (Positive) = More Risk Averse",
    caption = "Z-scores calculated relative to a fixed 30-year historical baseline to capture decadal cycles and prevent shifting baseline syndrome.",
    x = NULL,
    y = NULL
  ) +
...
file_name <- file.path(dir_images, "risk_score_heatmap_V6_30yr_baseline.png")
